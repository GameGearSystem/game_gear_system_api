...
import { register as RegisterUser, confirmation, getAll, getUser, getUserByEmail, getUserByResetToken, isUserConfirmed, updateConfirmationToken, updatePassword, updateResetToken } from "../services/user.service";

export const getMe = async (req: Request, res: Response) => {
  try {
    const user = await getUser(Number(req.user.userId["userId"]));
    res.status(200).json(createTResult(UserEntityToDto(user)));
  } catch (err) {
